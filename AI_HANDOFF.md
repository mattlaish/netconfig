...
- **`store.py` (core):** add `ConfigStore.search(pattern, *, devices=None,
  ...
  matched line, and optional context lines. Read files line by line and stop at
  the bounds; never load the archive into memory. Reuse the existing
  `_safe_name`/`_dir` helpers and `read_version`, whose path-traversal guard
  must remain the only way snapshot files are opened.
...
work is complete. Config archive full-text search is specified above but not
started: no code has been written for it, and no module outside this handoff
document has been touched for it.

## Recommended Next Step
Implement the config archive full-text search feature specified above: start
with `ConfigStore.search` and its offline tests in `selftest.py`, then the
`netconfig search` CLI subcommand, then the `/search` web page. It is
self-contained, read-only, and verifiable without devices or an RPM toolchain,
so it can be completed and tested in an ordinary Linux session.

Unchanged and still blocked on hardware: on an AlmaLinux 10 build/test VM, run
`packaging/build-rpm.sh`, inspect the resulting `-16` binary RPM/SRPM, upgrade
the recovered `-15` installation, and run `packaging/smoke-installed.sh`. Then
validate the expanded SNMP/MIB pages and reduced SNMPv3 CPU use against a real
device and a representative vendor MIB set.

## Last Verified
2026-08-21 in a Linux session with Python 3.12.3: full offline self-test,
`compileall`, and CLI `--help` all pass, and the working tree is unmodified by
those checks. The only change made in this session is this handoff document,
which records the selected next feature and its design. It is intentionally
left uncommitted for the user to manage with Git; no Git command was run. The
earlier 2026-08-19 Windows results above remain valid for the packaging static
checks. An existing `git-save-push.ps1` helper remains in the workspace and
contains interactive add/commit/push commands; it was not executed.
